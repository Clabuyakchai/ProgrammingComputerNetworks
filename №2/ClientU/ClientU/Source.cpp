...
int main(int argc, TCHAR* argv[])
{
	...
	try
	{
		...
		to.sin_port = htons(2000);                   // TCP-порт 2000
		...
		string hello = "Hello";   //буфер вывода
		...
		for (int i = 1; i <= counter; i++)
		{
			...
			if ((sendto(cC, mass, strlen(mass) + 1, NULL,
				(sockaddr*)&to, sizeof(to))) == SOCKET_ERROR)
				throw  SetErrorMsgText("recv:", WSAGetLastError());

			messageCliet = "";
			
			if ((recvfrom(cC, messageServer, sizeof(messageServer), NULL,
			...
			if ((sendto(cC, mass1, strlen(mass1) + 1, NULL,
