...
int main()
{
	...
	try 
	{
		...
		bool connectedWithServer = GetServerByName("Clabuyakchai", serverName, (sockaddr*)&serverParameters, &sizeOfServerParameters);
...
bool GetServerByName(char* name, char* call, struct sockaddr* serverParameters, int* flen)
{
	HOSTENT *hosten;
	hosten = gethostbyname(name);

	in_addr *ipAdress;
	ipAdress = (in_addr*)*(hosten->h_addr_list);

	((sockaddr_in*)serverParameters)->sin_family = AF_INET;
	((sockaddr_in*)serverParameters)->sin_port = htons(2000);
	((sockaddr_in*)serverParameters)->sin_addr = *ipAdress;
	...
	if ((countOfSendedBytes = sendto(clientSocket, call, strlen(call), NULL, (sockaddr*)&serversParameters, sizeof(serversParameters))) == SOCKET_ERROR)
		throw SetErrorMsgText("sendto: ", WSAGetLastError());

	if ((countOfReceivedBytes = recvfrom(clientSocket, messageFromServer, sizeof(messageFromServer), NULL, serverParameters, flen)) == SOCKET_ERROR)
		throw SetErrorMsgText("recvfrom: ", WSAGetLastError());

	return strcmp(call, messageFromServer) == 0;
}